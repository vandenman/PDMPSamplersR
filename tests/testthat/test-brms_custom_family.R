...
test_that("validate_phase1_surface passes for supported model", {
  model_block <- paste(
    "  if (!prior_only) {",
    "    vector[N] mu = rep_vector(0.0, N);",
    "    mu += Intercept + Xc * b;",
  ...
  expect_no_error(PDMPSamplersR:::validate_phase1_surface(model_block))
})

test_that("validate_phase1_surface rejects missing mu init", {
  model_block <- "  mu += Intercept + Xc * b;"
  expect_error(
    PDMPSamplersR:::validate_phase1_surface(model_block),
    "mu initialization"
  )
})

test_that("validate_phase1_surface rejects missing accumulation", {
  model_block <- "  vector[N] mu = rep_vector(0.0, N);"
  expect_error(
    PDMPSamplersR:::validate_phase1_surface(model_block),
    "no supported predictor accumulation"
  )
})

test_that("validate_phase1_surface rejects random effects", {
  model_block <- paste(
    "  vector[N] mu = rep_vector(0.0, N);",
    "  mu += Intercept + Xc * b;",
    "  for (n in 1:N) {",
    "    mu[n] += r_group_1[J_1[n]] * Z_1_1[n];",
    "  }",
    sep = "\n"
  )
  expect_error(
...
test_that("validate_phase1_surface rejects distributional predictor matrices", {
  model_block <- paste(
    "  vector[N] mu = rep_vector(0.0, N);",
    "  mu += Intercept + Xc * b;",
...
test_that("validate_phase1_surface accepts spline terms", {
  model_block <- paste(
    "  vector[N] mu = rep_vector(0.0, N);",
...
test_that("validate_phase1_surface accepts GP terms", {
  model_block <- paste(
...
test_that("validate_phase1_surface accepts FE + splines + GP combined", {
  model_block <- paste(
    "  vector[Nsubgp_1] gp_pred_1 = gp_exp_quad(Xgp_1, sdgp_1[1], lscale_1[1], zgp_1);",
    "  vector[N] mu = rep_vector(0.0, N);",
