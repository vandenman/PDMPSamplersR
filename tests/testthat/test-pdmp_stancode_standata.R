# Tests for pdmp_stancode() and pdmp_standata() transparency functions.
...
test_that("pdmp_stancode without subsampling matches brms::stancode", {
  skip_if_no_brms()

  df <- data.frame(y = rnorm(20), x = rnorm(20))
  brms_code <- brms::stancode(y ~ x, data = df)
  pdmp_code <- pdmp_stancode(y ~ x, data = df)
  expect_equal(pdmp_code, brms_code)
})

test_that("pdmp_stancode with subsampling returns a list with standard and ext_cpp", {
  skip_if_no_brms()

  df <- data.frame(y = rnorm(50), x = rnorm(50))
  result <- pdmp_stancode(y ~ x, data = df, subsample_size = 10L)
...
test_that("pdmp_stancode standard variant moves means_X to data block", {
  skip_if_no_brms()
...
test_that("pdmp_stancode with subsampling works for bernoulli", {
  skip_if_no_brms()

  df <- data.frame(y = rbinom(50, 1, 0.5), x = rnorm(50))
  result <- pdmp_stancode(y ~ x, data = df, family = brms::bernoulli(),
                          subsample_size = 10L)
  expect_true(is.list(result))
  expect_named(result, c("standard", "ext_cpp"))
...
test_that("pdmp_standata without subsampling matches brms::standata", {
  skip_if_no_brms()

  df <- data.frame(y = rnorm(20), x = rnorm(20))
  brms_data <- brms::standata(y ~ x, data = df)
  pdmp_data <- pdmp_standata(y ~ x, data = df)
  expect_equal(pdmp_data, brms_data)
})

test_that("pdmp_standata with subsampling returns three datasets", {
  skip_if_no_brms()

  set.seed(42)
  df <- data.frame(y = rnorm(50), x = rnorm(50))
  result <- pdmp_standata(y ~ x, data = df, subsample_size = 10L)

  expect_true(is.list(result))
  expect_named(result, c("full", "prior", "subsample"))
})

test_that("pdmp_standata full dataset has correct structure", {
  skip_if_no_brms()
...
test_that("pdmp_standata prior dataset has N=1 and prior_only=1", {
  skip_if_no_brms()
...
test_that("pdmp_standata subsample has correct size", {
  skip_if_no_brms()
...
test_that("pdmp_standata with explicit indices uses them", {
  skip_if_no_brms()

  df <- data.frame(y = 1:50 + 0.0, x = rnorm(50))
  idx <- c(1L, 5L, 10L, 15L, 20L)
  result <- pdmp_standata(y ~ x, data = df, subsample_size = 5L, indices = idx)

  expect_equal(result$subsample$N, 5L)
  expect_equal(as.numeric(result$subsample$Y), c(1, 5, 10, 15, 20))
})

test_that("pdmp_standata rejects mismatched indices length", {
  skip_if_no_brms()

  df <- data.frame(y = rnorm(50), x = rnorm(50))
  expect_error(
    pdmp_standata(y ~ x, data = df, subsample_size = 10L, indices = 1:5),
    "indices"
  )
})

test_that("pdmp_standata rejects subsample_size >= nrow(data)", {
  skip_if_no_brms()

  df <- data.frame(y = rnorm(10), x = rnorm(10))
  expect_error(
    pdmp_standata(y ~ x, data = df, subsample_size = 10L),
    "subsample_size"
  )
})

test_that("pdmp_standata rejects random effects with subsampling", {
  skip_if_no_brms()

  df <- data.frame(y = rnorm(20), x = rnorm(20), g = rep(1:4, each = 5))
  expect_error(
    pdmp_standata(y ~ x + (1 | g), data = df, subsample_size = 5L),
    "fixed-effects"
  )
})

test_that("pdmp_standata means_X matches column means of X", {
  skip_if_no_brms()

  set.seed(42)
  df <- data.frame(y = rnorm(50), x1 = rnorm(50), x2 = rnorm(50))
  result <- pdmp_standata(y ~ x1 + x2, data = df, subsample_size = 10L)
...
test_that("pdmp_standata works for bernoulli family", {
  skip_if_no_brms()

  df <- data.frame(y = rbinom(50, 1, 0.5), x = rnorm(50))
  result <- pdmp_standata(y ~ x, data = df,
...
test_that("pdmp_standata works for poisson family", {
  skip_if_no_brms()
...
test_that("pdmp_stancode and pdmp_standata are consistent", {
  skip_if_no_brms()

  df <- data.frame(y = rnorm(50), x = rnorm(50))
  code <- pdmp_stancode(y ~ x, data = df, subsample_size = 10L)
  dlist <- pdmp_standata(y ~ x, data = df, subsample_size = 10L)
...
test_that("pdmp_stancode ext_cpp injects external C++ declarations for gaussian", {
  skip_if_no_brms()
...
test_that("pdmp_stancode ext_cpp injects external C++ declarations for poisson", {
  skip_if_no_brms()

  df <- data.frame(y = rpois(50, 3), x = rnorm(50))
  result <- pdmp_stancode(y ~ x, data = df, family = poisson(),
                          subsample_size = 10L)
