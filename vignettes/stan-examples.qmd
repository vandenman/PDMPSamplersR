...
#| label: setup
#| include: false
# Guard: check Julia is actually on PATH before calling julia_setup(),
# because a segfault in JuliaCall::julia_setup() is not catchable by tryCatch.
julia_on_path <- nzchar(Sys.which("julia"))

julia_available <- julia_on_path && tryCatch(
    {
        ...
        check_for_julia_setup(error_if_not_exists = FALSE,
                              setup_if_not_exists = FALSE)
    ...
    error = function(e) {
        message("Vignette setup failed: ", conditionMessage(e))
        FALSE
    }
)
...
`pdmp_result` objects hold a pointer to the PDMP trace that lives in Julia.
This pointer only remains valid as long as the Julia process lives, so naively calling `saveRDS()`
on a bare result and later `readRDS()` leaves that pointer dangling and every subsequent estimator
call fails.

By default, all sampling functions (`pdmp_sample`, `pdmp_sample_from_stanmodel`,
`pdmp_sample_subsampled`) immediately copy the chain skeleton, i.e., the event times, positions, and
velocities, from Julia to R. This costs a little bit of extra time and memory but does mean that `saveRDS()` / `readRDS()` work
out of the box

If you want to skip the upfront copy, for example when running many short exploratory chains
that you will never save, or when memory constrained in a large model, pass `materialize = FALSE`:
...
You can still call `materialize()` manually at any later point to extract the skeleton
and enable `saveRDS` and `readRDS` functionality.
