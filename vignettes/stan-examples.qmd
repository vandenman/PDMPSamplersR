...
julia_available <- nzchar(Sys.which("julia"))
...
#| fig-width: 8
#| fig-height: 5
est_mean <- colMeans(result$samples)
est_cov  <- cov(result$samples)
...
#| fig-width: 8
#| fig-height: 5
est_mean <- colMeans(result2$samples)
est_cov  <- cov(result2$samples)
...
tib <- tibble(
  ...
  est  = colMeans(result3$samples != 0)
)
...
tib <- tibble(
  true = prior_incl_prob,
  est  = colMeans(result4$samples != 0)
)
...
#| fig-width: 10
#| fig-height: 5
samples <- result5$samples

tib_incl <- tibble(
  true = prior_incl_prob,
  est  = colMeans(samples != 0)
)
...
#| fig-width: 8
#| fig-height: 8
samples <- result_lr$samples

tib_lr <- tibble(
  true   = rep(c(intercept, beta), 2),
  est    = c(colMeans(samples), coef(fit)),
  how    = rep(c("Bayes", "Frequentist"), each = d + 1),
  p_incl = c(colMeans(samples != 0), rep(1, d + 1)),
